 ...
 */
class Customweb_Unzer_Communication_Type_ResponseProcessor extends Customweb_Unzer_Communication_AbstractTransactionProcessor {

	public function process(Customweb_Core_Http_IResponse $response){
		try {
			parent::process($response);
			$this->getPaymentMethodByTransaction($this->transaction)->processTypeData($this->transaction, $this->data);
		}
	...
	private function getFailedMessage(){
		return Customweb_I18n_Translation::__("Processing type failed");
	}
